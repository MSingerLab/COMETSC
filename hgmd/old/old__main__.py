...
def process(cls,X,L,plot_pages,cls_ser,tsne,marker_exp,gene_file,csv_path,vis_path,pickle_path,cluster_number,K,abbrev,cluster_overall,Trim,count_data):
    ...
    if ab in abbrev:
        ...
        for index,row in xlmhg.iterrows():
            ...
            if re.compile(".*_negation$").match(row['gene_1']):
                if sing_tp_tn.set_index('gene_1').at[row['gene_1'],'TN'] < .5:
                    continue
    ...
    sing_output_positives.to_csv(
        csv_path + '/cluster_' + str(cls) + '_singleton_positive_markers_only_ranked.csv'
    )
    ...
    marker_exp_cop = marker_exp.copy(deep=True)
    vis.make_plots(
        pair=ranked_pair,
        sing=sing_output,
